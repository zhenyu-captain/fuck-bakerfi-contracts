#!/bin/bash
# BakerFi 多版本合约 SlithIR 提取工具
# 支持 b-pre-mitigation, b-post-mitigation, latest 版本
# 使用方法: ./extract-slithir.sh [版本目录] [合约目录]
...
if [ $# -gt 0 ]; then
    if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
        ...
        echo ""
        echo "  合约目录选项:"
        echo "    core              - 提取核心合约 SlithIR (contracts/core/)"
        echo "    interfaces        - 提取接口合约 SlithIR (contracts/interfaces/)"
        echo "    libraries         - 提取库合约 SlithIR (contracts/libraries/)"
        echo "    oracles           - 提取预言机合约 SlithIR (contracts/oracles/)"
        echo "    proxy             - 提取代理合约 SlithIR (contracts/proxy/)"
        echo ""
        echo "示例:"
        ...
        echo ""
        echo "如果不指定合约目录，将提取所有合约的 SlithIR"
        echo "如果不指定版本目录，将自动检测当前目录的版本"
        echo ""
        echo "SlithIR 是 Slither 的中间表示（SSA 形式），包含："
        echo "  - 控制流信息"
...
echo -e "${BLUE}=== BakerFi SlithIR 提取工具 (${VERSION_TYPE}版本) ===${NC}"
...
# 设置输出目录（包含版本信息）
OUTPUT_DIR="$BASE_DIR/workspace/analysis_security_return/slither/slithir-${VERSION_TYPE}"
...
if [ -n "$CONTRACT_DIR" ]; then
    OUTPUT_FILE="${OUTPUT_DIR}/slithir-${VERSION_TYPE}-${CONTRACT_DIR}-${TIMESTAMP}.txt"
else
    OUTPUT_FILE="${OUTPUT_DIR}/slithir-${VERSION_TYPE}-all-${TIMESTAMP}.txt"
fi
    ...
    if [ -d "contracts/$CONTRACT_DIR" ]; then
        ...
        case "$CONTRACT_DIR" in
            "core")
                ...
                echo "  📁 提取目标: 核心合约 (contracts/core/)"
                ;;
            "interfaces")
                FILTER_PATHS="node_modules/,test/,mocks/,core/,libraries/,oracles/,proxy/"
                echo "  📁 提取目标: 接口合约 (contracts/interfaces/)"
                ;;
            "libraries")
                FILTER_PATHS="node_modules/,test/,mocks/,core/,interfaces/,oracles/,proxy/"
                echo "  📁 提取目标: 库合约 (contracts/libraries/)"
                ;;
            "oracles")
                FILTER_PATHS="node_modules/,test/,mocks/,core/,interfaces/,libraries/,proxy/"
                echo "  📁 提取目标: 预言机合约 (contracts/oracles/)"
                ;;
            "proxy")
                FILTER_PATHS="node_modules/,test/,mocks/,core/,interfaces/,libraries/,oracles/"
                echo "  📁 提取目标: 代理合约 (contracts/proxy/)"
                ;;
            *)
                echo -e "${YELLOW}⚠ 警告: 未知的合约目录 $CONTRACT_DIR，将提取所有合约的 SlithIR${NC}"
                ;;
        esac
    else
        echo -e "${YELLOW}⚠ 警告: 合约目录 contracts/$CONTRACT_DIR 不存在，将提取所有合约的 SlithIR${NC}"
    fi
else
    echo "  📁 提取目标: 所有合约"
fi
...
echo "  1. 检查生成的 SlithIR 文件"
echo "  2. 分析合约的控制流和底层操作"
echo "  3. 识别变量赋值和使用模式"
echo ""
echo -e "${BLUE}SlithIR 说明:${NC}"
echo "  SlithIR 是 Slither 的中间表示（SSA 形式），包含："
echo "  - 控制流信息"
echo "  - 底层操作（Low-level operations）"
echo "  - 变量赋值和使用"
