#!/bin/bash
# BakerFi 多版本合约函数摘要提取工具
# 支持 b-pre-mitigation, b-post-mitigation, latest 版本
# 使用方法: ./extract-function-summary.sh [版本目录] [合约目录]
...
if [ $# -gt 0 ]; then
    if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
        ...
        echo ""
        echo "  合约目录选项:"
        echo "    core              - 提取核心合约函数摘要 (contracts/core/)"
        echo "    interfaces        - 提取接口合约函数摘要 (contracts/interfaces/)"
        echo "    libraries         - 提取库合约函数摘要 (contracts/libraries/)"
        echo "    oracles           - 提取预言机合约函数摘要 (contracts/oracles/)"
        echo "    proxy             - 提取代理合约函数摘要 (contracts/proxy/)"
        echo ""
        echo "示例:"
...
echo -e "${BLUE}=== BakerFi 函数摘要提取工具 (${VERSION_TYPE}版本) ===${NC}"
...
# 设置输出目录（包含版本信息）
OUTPUT_DIR="$BASE_DIR/workspace/analysis_security_return/slither/function-summary-${VERSION_TYPE}"
...
if [ -n "$CONTRACT_DIR" ]; then
    OUTPUT_FILE="${OUTPUT_DIR}/function-summary-${VERSION_TYPE}-${CONTRACT_DIR}-${TIMESTAMP}.txt"
else
    OUTPUT_FILE="${OUTPUT_DIR}/function-summary-${VERSION_TYPE}-all-${TIMESTAMP}.txt"
fi
    ...
    if [ -d "contracts/$CONTRACT_DIR" ]; then
        ...
        case "$CONTRACT_DIR" in
            ...
            "proxy")
                ...
                ;;
            *)
                echo -e "${YELLOW}⚠ 警告: 未知的合约目录 $CONTRACT_DIR，将提取所有合约的函数摘要${NC}"
                ;;
        esac
    else
        echo -e "${YELLOW}⚠ 警告: 合约目录 contracts/$CONTRACT_DIR 不存在，将提取所有合约的函数摘要${NC}"
    fi
else
    echo "  📁 提取目标: 所有合约"
fi
